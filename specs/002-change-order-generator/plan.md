...
**Created**: 2025-11-11
**Status**: In Progress (Phase 3 complete, Phase 4-5 pending)
**Spec Version**: Clarified
**Last Updated**: 2025-11-15 (Post-PR #6, #7, #8 audit)

## Implementation Status

**Completed PRs:**
- ✅ PR #6: Phase 1-2 (Foundation + Settings) - 60/64 tasks (94%)
- ✅ PR #7: Popup refactoring (quality improvement, not in spec)
...
1. **Architecture Simplifications:**
   - Template engine split into 3 modular files (TemplateEngine, TemplateProcessor, TemplateHelpers) instead of single file
   - Plain text template instead of HTML for MVP
   - ChangeOrderBuilder returns formatted string, not ChangeOrder entity
   - No separate change-order/ UI directory - integrated into popup

2. **Deferred to Later Phases:**
   - Change order history (Phase 7)
   - Sequential numbering per client
   - Original scope pre-fill from history
   - Payment terms, revised timeline, signatures sections
   - Auto-export feature (Phase 6)
   - Draft persistence (Phase 7)
   - PDF/clipboard/text export (Phase 5)

3. **Key Architectural Decisions:**
   - Calculator logic inline in ChangeOrderBuilder (no separate PricingCalculator.js)
   - Multi-item selection integrated into DetectionListRenderer (no separate component)
   - Template inline in getTemplate() method (no external HTML file)
   - First-run handled by WelcomeModal + FirstRunDetector

**Current State:**
- Core generation working: detections → formatted text document
- Settings page functional: freelancerName, hourlyRate saveable
- Template engine fully functional: {{variables}}, {{@if}}, {{@each}}
- Multi-item selection working: checkboxes in detection list
- Cost calculation working: hourlyRate × hours if rate provided

**Next Phase (PR #9):**
- Phase 4: Pricing calculator widget UI
- Phase 5: Export functionality (PDF, clipboard, text)
...
### Entities & Relationships - ACTUAL IMPLEMENTATION

**ChangeOrder** (entity class):
- File: `src/lib/change-order/ChangeOrder.js`
- Fields implemented:
  - id (UUID via generateUUID())
  - changeOrderNumber (string, e.g., "001")
  - clientName, clientEmail
  - freelancerName (from settings)
  - dateCreated (ISO 8601 via getCurrentDateTimeISO())
  - originalScope, requestedChanges (array)
  - costEstimate, revisedTimeline, paymentTerms, additionalNotes
  - status ('draft', 'generated', 'exported')
  - exportedAt, exportFormat
- Methods: validate(), toJSON(), fromJSON(), markAsExported(), canExport()
- Validation: Checks required fields, status values, export fields
- **NOT USED FOR MVP**: Change orders aren't saved to storage yet - only used as data model

**ChangeOrderBuilder** (service class):
- File: `src/lib/change-order/ChangeOrderBuilder.js`
- Purpose: Builds change order documents from detections
- Key method: `build(detections, settings)` returns formatted text document
- Features:
  - Estimates hours (2h per detection heuristic)
  - Calculates costs if hourly rate available
  - Generates plain text document using template
  - Handles empty detections gracefully
- **SIMPLIFICATION**: Returns plain text string, not ChangeOrder entity

**TemplateEngine** (template processor):
- Files: `TemplateEngine.js`, `TemplateProcessor.js`, `TemplateHelpers.js`
- Features:
...
- Security: String replacement only (RegExp-based), safe for user data
- Performance: <500ms for 50-item templates
- **ARCHITECTURAL DECISION**: Modular design (3 files) for maintainability

**FreelancerSettings** (entity class):
- File: `src/lib/storage/FreelancerSettings.js`
- Fields implemented:
  - freelancerName (required)
  - hourlyRate (optional, number)
  - defaultExportMethod ('clipboard', 'pdf', 'text')
  - autoExportEnabled, autoExportDelay
  - lastUpdated
- Methods: validate(), toJSON(), fromJSON(), getDefaults(), isFirstRun(), update()
- Storage: Saved to chrome.storage.local via SettingsStorage service
- **INTEGRATED**: Used by ChangeOrderBuilder for pre-filling data

**Template Format** (plain text):
- Location: Inline in `ChangeOrderBuilder.getTemplate()` method
- Format: Plain text with template placeholders (not HTML)
- Sections:
  - Header: Generated date, freelancer name
  - Scope creep detections list (with {{@each}} loop)
  - Summary: Total items, hours, cost (if hourly rate available)
  - Next steps guidance
  - Footer: "Generated by ScopeShield"
- **SIMPLIFICATION**: Plain text instead of HTML for MVP

**ExportHistory** (deferred):
- Not implemented in MVP
- Planned for Phase 7
- Will track export events for analytics
...
1. **Change Order Generation** (in `src/popup/components/DetectionEventHandlers.js`):
   ```javascript
   // Actual implementation
   async generateReport() {
     const detections = this.getEvents();
     ...
     const changeOrderText = await builder.build(detections, settings);

     // Display in modal
     this.showChangeOrderModal(changeOrderText);
   }
   ```

2. **Detection Selection** (multi-item bundling):
   - Implemented in `DetectionListRenderer.js` - checkboxes on each detection
   - User can select multiple detections before generating
   - All selected detections combined into single change order
   - Detection fields used: sender, text, trigger, date

**With Settings System**:
- Settings loaded via `FreelancerSettings.load()` which wraps SettingsStorage
- Returns defaults if no settings exist (first-run)
- Settings used by ChangeOrderBuilder:
  - `freelancerName`: Pre-fills change order author
  - `hourlyRate`: Used for cost calculation (if > 0)
- Settings page: `src/popup/settings/SettingsView.js`
  - Form: `SettingsForm.js` + `SettingsFormTemplate.js`
  - Validation before save
  - Success/error notifications

**First-Run Flow**:
- `FirstRunDetector.js` checks if freelancerName is empty
- `WelcomeModal.js` shown on first popup open
- Prompts user for name
- Saves to settings via `FirstRunDetector.completeFirstRun(name)`
- Blocks change order generation until name entered

**Available APIs for Phase 4 & 5**:
1. `ChangeOrderBuilder.build(detections, settings)`:
   - Returns: Formatted text document (string)
   - Performance: <5s for 50 detections

2. `FreelancerSettings.load()`:
   - Returns: Settings object with hourlyRate, freelancerName, etc.
   - Already has validation built-in

3. `TemplateEngine.render(template, data)`:
   - Supports {{variables}}, {{@if}}, {{@each}}
   - Can be used for custom templates

4. Detection list integration:
   - Multi-select already implemented in DetectionListRenderer
   - Checkbox state managed by popup.js
...
## Phase 4 & 5 Integration Guidance (for PR #9)

### Available APIs

**1. ChangeOrderBuilder (core generation):**
```javascript
...
// Builder provides:
builder.estimateHours(detections); // Returns: number (2h per detection)
builder.calculateCost(hours, rate); // Returns: number
builder.formatDate(dateString); // Returns: 'YYYY-MM-DD'
```

**2. FreelancerSettings (user preferences):**
```javascript
import { FreelancerSettings } from '../lib/storage/FreelancerSettings.js';

const settings = await FreelancerSettings.load();
// Returns: { freelancerName, hourlyRate, defaultExportMethod, ... }

settings.hourlyRate; // Number (0 if not set)
settings.freelancerName; // String
settings.defaultExportMethod; // 'clipboard' | 'pdf' | 'text'
```

**3. TemplateEngine (for custom rendering):**
```javascript
...
**1. Calculator Widget (Phase 4):**
- Location: Inline in change order modal (DetectionEventHandlers.showChangeOrderModal)
- Data source: `settings.hourlyRate` (pre-fill), detections.length * 2 (hours)
- Update: Modify modal HTML to include calculator section
- State management: Store in modal DOM, update on input change

**2. Export Controls (Phase 5):**
- Location: Bottom of change order modal
- Available data: `changeOrderText` (the formatted document)
- Required exports:
  - **Clipboard**: `navigator.clipboard.writeText(changeOrderText)`
  - **PDF**: Use jsPDF to convert text to PDF (lazy-loaded)
  - **Text**: Same as clipboard (download as .txt file)

**3. Modal Structure:**
```html
<!-- Current modal (in DetectionEventHandlers.js) -->
<div class="modal">
  <pre>{{changeOrderText}}</pre>

  <!-- ADD FOR PHASE 4: Calculator Widget -->
  <div class="calculator-widget">
    <input id="hourly-rate" value="{{settings.hourlyRate}}" />
    <input id="estimated-hours" value="{{detections.length * 2}}" />
    <div class="calculated-cost">$<span id="cost">{{rate * hours}}</span></div>
  </div>

  <!-- ADD FOR PHASE 5: Export Buttons -->
...
### Data Flow for Phase 4 & 5

```
User clicks "Generate Change Order"
  ↓
DetectionEventHandlers.generateReport()
  ↓
FreelancerSettings.load() → settings
  ↓
ChangeOrderBuilder.build(detections, settings) → document text
  ↓
showChangeOrderModal(document)
  ↓
[PHASE 4] Calculator widget shows rate × hours
  ↓
User adjusts calculator OR accepts defaults
  ↓
[PHASE 5] User clicks export button
  ↓
ExportService handles export (PDF/Clipboard/Text)
  ↓
Success notification shown
```

### Files to Modify (PR #9)

**Phase 4 (Calculator Widget):**
1. `src/popup/components/DetectionEventHandlers.js`:
   - Add calculator HTML to modal template
   - Add input handlers for rate/hours changes
   - Update cost display on input change

2. `src/popup/popup.css`:
   - Add `.calculator-widget` styles
   - Input field styling
   - Cost display styling

**Phase 5 (Export Functionality):**
1. Create `src/lib/change-order/PDFGenerator.js`:
   - Lazy-load jsPDF
   - Convert text to PDF format
   - Return blob for download

2. Create `src/lib/change-order/ExportService.js`:
   - exportToClipboard(text)
   - exportToPDF(text)
   - exportToText(text)
   - Handle errors with graceful fallbacks

3. Update `src/popup/components/DetectionEventHandlers.js`:
   - Add export button handlers
   - Call ExportService methods
   - Show success/error notifications

### Testing Checklist (PR #9)

**Calculator Widget:**
- [ ] Pre-fills hourlyRate from settings
- [ ] Calculates cost correctly (rate × hours)
- [ ] Updates in real-time on input change
- [ ] Handles rate = 0 (no cost shown)
- [ ] Input validation (rate ≥ 0, hours > 0)

**Export Functionality:**
- [ ] Clipboard copy works (<500ms)
- [ ] PDF export works (<3s)
- [ ] Text download works
...
**Last Updated**: 2025-11-15 (Post-implementation audit, added integration guidance)
**Ready for Task Breakdown**: ✅ YES - Tasks.md updated with completion status
